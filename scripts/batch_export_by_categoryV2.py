...
def export_combined_glb(storey_categories, output_path):
    """Export all storeys as one GLB with storey groups for AR toggle"""
    ...
    for storey, categories in storey_categories.items():
        ...
        for category, meshes in categories.items():
            ...
            try:
                ...
                # Post-process: ensure Y-up orientation in exported GLB
post_mesh = trimesh.load(filepath)
if hasattr(post_mesh, 'geometry'):
    for name, geom in post_mesh.geometry.items():
        if hasattr(geom, 'vertices'):
            # Check if Z is up (extents)
            extents = geom.extents
            if extents[2] > extents[1]:
                # Z is taller than Y — rotate -90° around X
                rotation = np.array([
                    [1,  0,  0],
                    [0,  0,  1],
                    [0, -1,  0]
                ])
                geom.vertices = np.dot(geom.vertices, rotation.T)
                print(f"    🔄 Rotated {name} to Y-up")
    
    # Re-export with rotation
    post_mesh.export(filepath, file_type='glb')
